...
class Patricia {
 public:
  ...
  ~Patricia() { Destroy_Patricia(tree); }

  template <typename Type>
  patricia_node_t *add_ref(const char *string, Type *val) {
  ...
  // template <typename Type> patricia_node_t *add(const char *string, Type
  // *val);
  template <typename Type>
  patricia_node_t *add(int family, const char *string, Type *val) {
  ...
  void *get(uint32_t addr) { return (get(addr, false)); }
  void *get(struct in6_addr addr);
  void *get(int family, const char *string, bool exact);
  void *get(const char *string) { return (get(AF_INET, string, false)); }
  ...
  void populate(const char *filename) { populate(AF_INET, filename); };

  void populate6(const char *filename) { populate(AF_INET6, filename); };
...
// Export the library
extern "C" {
Patricia *Patricia_new(uint8_t size);
void Patricia_populate(Patricia *patricia, const char *bgp_file);
void *Patricia_get(Patricia *patricia, uint32_t addr);
}
