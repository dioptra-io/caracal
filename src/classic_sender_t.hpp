...
class classic_sender_t {
 public:
  classic_sender_t(uint8_t family, int type, uint8_t proto,
                   const std::string &src_addr, const uint32_t pps);
  void send(int n_packets, uint32_t destination, uint8_t ttl, uint16_t sport,
            uint16_t dport);
