...
void packets_utils::complete_ip_header(uint8_t *ip_buffer, uint32_t destination,
...
void packets_utils::add_transport_checksum(uint8_t *transport_buffer,
...
void packets_utils::add_tcp_ports(uint8_t *transport_buffer, uint16_t sport,
                                  uint16_t dport) {
  tcphdr *tcp_header = reinterpret_cast<tcphdr *>(transport_buffer);
...
void packets_utils::add_tcp_timestamp(uint8_t *transport_buffer, timeval &start,
  ...
  //    if (time_diff < 500){
  //        std::cout << "Reset start time: " << start.tv_sec << "." <<
  //        start.tv_usec << " seconds since epoch." << std::endl; std::cout <<
  //        time_diff << " " << seq_no << std::endl;
  //    }

  tcp_header->th_seq = htonl(seq_no);
}

void packets_utils::add_udp_length(uint8_t *transport_buffer,
...
void packets_utils::add_udp_timestamp(uint8_t *transport_buffer,
                                      uint8_t *ip_buffer,
  ...
  uint32_t pseudo_header_sum_16 =
      in_cksum(reinterpret_cast<uint8_t *>(&psh), sizeof(pseudo_header_udp), 0);

  uint8_t *data =
      reinterpret_cast<uint8_t *>(transport_buffer + sizeof(udphdr));
