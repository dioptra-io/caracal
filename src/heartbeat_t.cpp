...
heartbeat_t::heartbeat_t(const std::string &interface,
...
bool heartbeat_t::check_destination_ttl(uint32_t little_endian_addr,
                                        uint8_t ttl, uint32_t host_offset) {
  ...
  if (!(m_options.inf_born < little_endian_addr &&
        little_endian_addr <= m_options.sup_born)) {
    return false;
  }
  // Flow starting at 0
...
void heartbeat_t::send_from_probes_file() {
  ...
  pf_ring_sender_t sender{AF_INET,
                          ...
                          m_options.pps};
#else
  classic_sender_t sender{AF_INET, SOCK_DGRAM, m_options.proto,
                          source.to_string(), m_options.pps};
#endif
  ...
  while (std::getline(targets_file, line)) {
    ...
    if (m_patricia_trie_excluded.get(dst_ip) != nullptr) {
      //            std::cout << "Not sending to excluded: " <<
      //            IPv4Address(addr) << "\n";
      continue;
    }
...
void heartbeat_t::send_exhaustive() {
  ...
  while (PERM_END != cperm_next(perm, &val)) {
     ...
     *  24 bits for the network, 5 bits for TTL (up to 30) and 3 bits for the
     * MDA destination based (7 at max) corresponding to at max 0.05 failure
     * probability. If 0.01 failure probability is needed, when TTL is 0 or 31,
     * switch this bit to the flow id.
     */
    ...
    // 20201112 Maxime: Reverse byte order in an attempt to distribute dest.
    // addrs. more evenly, and discover more routers close to the destination.
  ...
  std::cout << "Sending " << m_options.n_destinations_per_24 << " per /24 took "
            << elapsed.count() << " ms\n";
...
void heartbeat_t::send_from_targets_file(uint8_t max_ttl) {
  ...
  std::cout << "Number of bits needeed to encode " << n_targets
            << " targets: " << a << "\n";
  ...
  while (PERM_END != cperm_next(perm, &val)) {
    uint32_t addr_index =
        val &
        ((1 << a) -
         1);  // pick out a bits of the index of the address in the targets

    ttl = static_cast<uint8_t>((val >> a) &
                               ((1 << 5) - 1));  // pick out 5 bits for the TTL.
...
void heartbeat_t::start() {
  ...
  // Allows sniffer to get the last flying responses. 60 seconds maximum waiting
  // time.
